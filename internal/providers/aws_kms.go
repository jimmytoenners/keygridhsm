...
func (c *AWSKMSClient) Sign(ctx context.Context, keyHandle string, digest []byte, algorithm string) ([]byte, error) {
...
func (c *AWSKMSClient) Verify(ctx context.Context, keyHandle string, digest, signature []byte, algorithm string) (bool, error) {
	// Convert algorithm to AWS KMS signing algorithm
	signingAlgorithm, err := c.convertAlgorithmToAWS(algorithm)
	if err != nil {
		return false, err
...
func (c *AWSKMSClient) Encrypt(ctx context.Context, keyHandle string, plaintext []byte) ([]byte, error) {
...
func (c *AWSKMSClient) Decrypt(ctx context.Context, keyHandle string, ciphertext []byte) ([]byte, error) {
...
func (c *AWSKMSClient) WrapKey(ctx context.Context, keyHandle, wrappingKeyHandle string) ([]byte, error) {
	// AWS KMS uses GenerateDataKey for key wrapping
	input := &kms.GenerateDataKeyInput{
		KeyId:   ptrString(wrappingKeyHandle),
		KeySpec: types.DataKeySpecAes256,
	}

	result, err := c.client.GenerateDataKey(ctx, input)
	if err != nil {
		return nil, models.NewHSMErrorWithCause(models.ErrCodeKeyWrappingFailed,
...
func (c *AWSKMSClient) UnwrapKey(ctx context.Context, wrappedKey []byte, wrappingKeyHandle string) ([]byte, error) {
	input := &kms.DecryptInput{
	...
	result, err := c.client.Decrypt(ctx, input)
	if err != nil {
		return nil, models.NewHSMErrorWithCause(models.ErrCodeKeyUnwrappingFailed,
...
func (c *AWSKMSClient) convertKeySpecToAWS(spec models.KeySpec) (types.KeyUsageType, types.KeySpec, error) {
	...
	} else if contains(spec.Usage, "sign") || contains(spec.Usage, "verify") {
		keyUsage = types.KeyUsageTypeSignVerify
	} else if contains(spec.Usage, "encrypt") || contains(spec.Usage, "decrypt") {
